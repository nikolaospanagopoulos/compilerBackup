...
void vector_free(struct vector *vector) {
  free(vector->data);
  if (vector->saves) {
    // Free all saved vectors
    while (vector_count(vector->saves) > 0) {
      struct vector *saved_vector = vector_back(vector->saves);
      vector_pop(vector->saves);
      vector_free(saved_vector);
    }
...
void vector_shift_right_in_bounds(struct vector *vector, int index,
                                  int amount) {
