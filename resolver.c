...
void resolver_follow_part(struct resolver_process *resolver, struct node *node,
                          struct resolver_result *result);
struct resolver_entity *resolver_follow_exp(struct resolver_process *resolver,
                                            struct node *node,
                                            struct resolver_result *result);
...
resolver_follow_array_bracket(struct resolver_process *resolver,
                              ...
                              struct resolver_result *result);
struct resolver_entity *
resolver_follow_part_return_entity(struct resolver_process *resolver,
                                   struct node *node,
                                   struct resolver_result *result);
...
struct resolver_entity *resolver_entity_clone(struct resolver_entity *entity) {
  ...
  struct resolver_entity *new_entity =
      calloc(1, sizeof(struct resolver_entity));
  memcpy(new_entity, entity, sizeof(struct resolver_entity));
  vector_push(cp->gb, &new_entity);
  return new_entity;
}
...
struct resolver_result *resolver_new_result(struct resolver_process *process) {
  ...
  result->array_data.array_entities = arrayEntities;
  return result;
}
...
resolver_new_process(struct compile_process *compiler,
  ...
  compiler->trackedScopes = vector_create(sizeof(struct resolver_scope *));
...
void resolver_new_entity_for_rule(struct resolver_process *process,
  ...
  struct resolver_entity *entity_rule =
...
struct resolver_entity *resolver_get_entity_in_scope_with_entity_type(
    struct resolver_result *result, struct resolver_process *resolver,
  ...
  if (result && result->last_struct_union_entity) {
    ...
    int offset =
        struct_offset(resolver_compiler(resolver), node_var_datatype->type_str,
                      entity_name, &out_node, 0, 0);
    if (node_var_datatype->type == DATA_TYPE_UNION) {
      offset = 0;
    }
    return resolver_make_entity(
        resolver, result, NULL, out_node,
...
resolver_get_entity_for_type(struct resolver_result *result,
  ...
  while (scope) {
    entity = resolver_get_entity_in_scope_with_entity_type(
        result, resolver, scope, entity_name, entity_type);
    if (entity) {
      break;
    }
...
struct resolver_entity *resolver_get_entity_in_scope(
  ...
  return resolver_get_entity_in_scope_with_entity_type(result, resolver, scope,
                                                       entity_name, -1);
}
...
struct resolver_entity *resolver_get_variable(struct resolver_result *result,
                                              struct resolver_process *resolver,
                                              const char *var_name) {
  return resolver_get_entity_for_type(result, resolver, var_name,
...
struct resolver_entity *resolver_get_function_in_scope(
  ...
  return resolver_get_entity_for_type(result, resolver, func_name,
...
struct resolver_entity *
resolver_follow_for_name(struct resolver_process *resolver, const char *name,
                         struct resolver_result *result) {
  struct resolver_entity *entity =
      resolver_entity_clone(resolver_get_entity(result, resolver, name));
  if (!entity) {
    return NULL;
  }
...
struct resolver_entity *
resolver_follow_identifier(struct resolver_process *resolver, struct node *node,
                           struct resolver_result *result) {
  struct resolver_entity *entity =
      resolver_follow_for_name(resolver, node->sval, result);
  if (entity) {
...
struct resolver_entity *
resolver_follow_variable(struct resolver_process *resolver,
                         struct node *var_node,
                         struct resolver_result *result) {
  struct resolver_entity *entity =
      resolver_follow_for_name(resolver, var_node->var.name, result);
  return entity;
}

struct resolver_entity *
resolver_follow_struct_exp(struct resolver_process *resolver, struct node *node,
                           struct resolver_result *result) {
  // a.b
...
struct resolver_entity *resolver_follow_array(struct resolver_process *resolver,
                                              struct node *node,
                                              struct resolver_result *result) {
  resolver_follow_part(resolver, node->exp.left, result);
  struct resolver_entity *left_entity = resolver_result_peek(result);
...
struct datatype *resolver_get_datatype(struct resolver_process *resolver,
                                       struct node *node) {
...
void resolver_build_function_call_arguments(
    struct resolver_process *resolver, struct node *argument_node,
  ...
  if (is_argument_node(argument_node)) {
    ...
    resolver_build_function_call_arguments(resolver, argument_node->exp.left,
                                           root_func_call_entity,
                                           total_size_out);
    // Build the right node for this function argument
    resolver_build_function_call_arguments(resolver, argument_node->exp.right,
                                           root_func_call_entity,
                                           total_size_out);
  } else if (argument_node->type == NODE_TYPE_EXPRESSION_PARENTHESES) {
    resolver_build_function_call_arguments(
        resolver, argument_node->parenthesis.exp, root_func_call_entity,
        total_size_out);
  } else if (node_valid(argument_node)) {
    vector_push(root_func_call_entity->func_call_data.arguments,
                &argument_node);
...
struct resolver_entity *
resolver_follow_function_call(struct resolver_process *resolver,
                              struct resolver_result *result,
                              struct node *node) {
  resolver_follow_part(resolver, node->exp.left, result);
  struct resolver_entity *left_entity = resolver_result_peek(result);
  ...
  resolver_build_function_call_arguments(
      resolver, node->exp.right, func_call_entity,
      &func_call_entity->func_call_data.stack_size);
...
struct resolver_entity *
resolver_follow_parentheses(struct resolver_process *resolver,
                            struct node *node, struct resolver_result *result) {
...
void resolver_array_bracket_set_flags(struct resolver_entity *bracket_entity,
                                      struct datatype *dtype,
                                      struct node *bracket_node, int index) {
...
resolver_follow_array_bracket(struct resolver_process *resolver,
  ...
  struct resolver_entity *array_bracket_entity =
      resolver_create_new_entity_for_array_bracket(result, resolver, node,
...
struct resolver_entity *
resolver_follow_exp_parenthesis(struct resolver_process *resolver,
                                struct node *node,
                                struct resolver_result *result) {
  return resolver_follow_part_return_entity(resolver, node->parenthesis.exp,
                                            result);
}

struct resolver_entity *
resolver_follow_unsupported_unary_node(struct resolver_process *resolver,
                                       struct node *node,
                                       struct resolver_result *result) {
  return resolver_follow_part_return_entity(resolver, node->unary.operand,
                                            result);
}

struct resolver_entity *
resolver_follow_unsupported_node(struct resolver_process *resolver,
                                 struct node *node,
                                 struct resolver_result *result) {
  bool followed = false;
  switch (node->type) {
...
struct resolver_entity *resolver_follow_cast(struct resolver_process *resolver,
                                             struct node *node,
                                             struct resolver_result *result) {
  ...
  struct resolver_entity *cast_entity = resolver_create_new_cast_entity(
      resolver, operand_entity->scope, &node->cast.dtype);
  resolver_result_entity_push(result, cast_entity);
  return cast_entity;
}

struct resolver_entity *
resolver_follow_indirection(struct resolver_process *resolver,
                            struct node *node, struct resolver_result *result) {
...
resolver_follow_unary_address(struct resolver_process *resolver,
  ...
  struct resolver_entity *unary_address_entity =
      resolver_create_new_unary_get_address_entity(
          resolver, result, &last_entity->dtype, node, last_entity->scope,
          last_entity->offset);
  resolver_result_entity_push(result, unary_address_entity);
  return unary_address_entity;
}

struct resolver_entity *resolver_follow_unary(struct resolver_process *resolver,
                                              struct node *node,
                                              struct resolver_result *result) {
...
resolver_follow_part_return_entity(struct resolver_process *resolver,
  ...
  default: {
    // Can't do aanything lets create a special entity that requires more
    // computation later on at runtime.
...
void resolver_execute_rules(struct resolver_process *resolver,
                            struct resolver_result *result) {
  struct vector *saved_entities =
      vector_create(sizeof(struct resolver_entity *));
  vector_push(cp->gbForVectors, &saved_entities);
...
struct resolver_entity *resolver_merge_compile_time_result(
  ...
  if (left_entity && right_entity) {
    ...
    struct resolver_entity *result_entity = resolver->callbacks.merge_entities(
        resolver, result, left_entity, right_entity);
    if (!result_entity) {
      goto no_merge_possible;
    }
...
void _resolver_merge_compile_times(struct resolver_process *resolver,
                                   struct resolver_result *result) {
  struct vector *saved_entities =
      vector_create(sizeof(struct resolver_entity *));
  while (1) {
    ...
    // The left entity goes back to the result as we may be able to merge with
    // next entity.
...
void resolver_merge_compile_times(struct resolver_process *resolver,
                                  struct resolver_result *result) {
...
void resolver_finalize_result_flags(struct resolver_process *resolver,
                                    struct resolver_result *result) {
...
void resolver_finalize_unary(struct resolver_process *resolver,
                             struct resolver_result *result,
                             struct resolver_entity *entity) {
...
void resolver_finalize_last_entity(struct resolver_process *resolver,
                                   struct resolver_result *result) {
  struct resolver_entity *last_entity = resolver_result_peek(result);
...
void resolver_finalize_result(struct resolver_process *resolver,
                              struct resolver_result *result) {
